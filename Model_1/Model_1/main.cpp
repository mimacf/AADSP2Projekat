...
DSPfract outputBUffer[MAX_NUM_CHANNEL][BLOCK_SIZE];
...
DSPint main(DSPint argc, char* argv[])
{
	...
	//-------------------------------------------------	
	{
		...
		{	
			...
			processing(sampleBuffer, outputBUffer, &stanje, mod, enable);

			for(DSPint j=0; j<BLOCK_SIZE; j++)
			{
				for(DSPint k=0; k<outputWAVhdr.fmt.NumChannels; k++)
				{	
					sample = outputBUffer[k][j] * SAMPLE_SCALE ;	// crude, non-rounding 			
